...
def get_transform(size, transform_type="weak", min_visibility=0):
    ...
    """
    bbox_params= albu.BboxParams(format='coco', min_visibility=min_visibility, label_fields=['category_id'])
    # bbox_params = {
    #     'format': 'coco',
    ...
    augs = {'strong': albu.Compose([
        ...
        albu.OneOf([
            ...
            albu.ImageCompression(quality_lower=50),
        ], p=0.2),
        albu.OneOf([
            albu.RandomRain(),
            # albu.RandomSunFlare(),
    ...
    pipeline = albu.Compose([aug_fn,
                             # ])
                             normalize], bbox_params=bbox_params)
